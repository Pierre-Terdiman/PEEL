...
/*
PINT Plugin for Newton 3.14's master version (trunk from Jan 3, 2017).
...
The Newton API has changed here and there so the previous PINT plugin didn't compile anymore.
I adapted athe code as I could but there might be errors.
*/

#include "stdafx.h"
#include "PINT_Newton3_14.h"
...
public:
	...
	void CalculatePickForceAndTorque (const NewtonBody* const body, dFloat timestep, dVector& force, dVector& torque)
	{
		...
		if (((m_extLinearImpulse.DotProduct3(m_extLinearImpulse)) != 0.0f) || ((m_extAngularImpulse.DotProduct3(m_extAngularImpulse)) != 0.0f)) {
			dVector com; 
			dMatrix matrix; 
			dVector omega0;
			dVector veloc0;
...
// *int* model - model of operation 0 = exact, 1 = adaptive, n = linear. The default is adaptive.
	NewtonSetSolverModel(mWorld, gSolverIterationCount);	// TODO: better UI names for the different modes
//	NewtonSetFrictionModel(mWorld, 1);						// TODO: expose this to UI
//	NewtonSetSolverConvergenceQuality(mWorld, 1);			// TODO: expose this to UI
	...
	NewtonMaterialSetCollisionCallback (mWorld, materialNodeID, materialNodeID, OnAABBOverlap, OnContactsProcess);

	// set constructor and destructor for collision shapes 
	NewtonWorldSetCollisionConstructorDestructorCallback (mWorld, DuplicateCollisionShape, DestroyCollisionShape);
...
//	printf("\n%f | %f | %f\n", inertiaMatrix.m_front.m_x, inertiaMatrix.m_front.m_y, inertiaMatrix.m_front.m_z);
//	printf("%f | %f | %f\n", inertiaMatrix.m_up.m_x, inertiaMatrix.m_up.m_y, inertiaMatrix.m_up.m_z);
//	printf("%f | %f | %f\n", inertiaMatrix.m_right.m_x, inertiaMatrix.m_right.m_y, inertiaMatrix.m_right.m_z);
	dVector angularMomentum (inertiaMatrix.RotateVector(omega));
	torque += angularMomentum.CrossProduct(omega);
...
NewtonCollision* NewtonPint::CreateShape(NewtonWorld* const world, const PINT_SHAPE_CREATE* const shape, const PintShapeRenderer* const renderer, int groupID) const
{
	...
	switch (shape->mType)
	{
		...
		case PINT_SHAPE_CAPSULE:
		{
			...
			ret = NewtonCreateCapsule (world, collShape->mRadius, collShape->mRadius, collShape->mHalfHeight * 2.0f, 0, &localPose.m[0][0]);
			break;
		}
...
*/
		case PINT_SHAPE_CYLINDER: 
		{
			...
			ret = NewtonCreateCylinder(world, collShape->mRadius, collShape->mRadius, collShape->mHalfHeight * 2.0f, 0, &localPose.m[0][0]);
			break;
		}
...
NewtonBody* NewtonPint::CreateBody(const PINT_OBJECT_CREATE& desc, NewtonCollision* const shape, const dFloat* const worldPose, bool ccdMode)
{
	const dFloat* const veloc = &desc.mLinearVelocity.x;
	const dFloat* const omega = &desc.mAngularVelocity.x;
...
/*
	...
	PinkRigidBodyCookie* const cookie = new PinkRigidBodyCookie(desc.mKinematic);
...
PintObjectHandle NewtonPint::CreateObject(const PINT_OBJECT_CREATE& desc, void* creationHintHandle)
{
	...
	if (creationHintHandle) {
		...
		ret = CreateBody(desc, shape, &pose.m[0][0], gUseCCD);
		NewtonDestroyCollision(shape);
	}
	return ret;
}
...
PintJointHandle NewtonPint::CreateJoint(const PINT_JOINT_CREATE& desc)
{
	...
	switch (desc.mType) 
	{
		...
		case PINT_JOINT_HINGE:		// Hinge joints a.k.a. revolute joints
		{
			...
			hinge->SetLimits (jc.mMinLimitAngle, jc.mMaxLimitAngle);
			joint = hinge;
			break;
		}
...
/*		case PINT_JOINT_UNIVERSAL:		
		...
		case PINT_JOINT_PRISMATIC:	// Prismatic joints, a.k.a. slider constraints
		{
			...
			{
				slider->SetLimits(jc.mMinLimit, jc.mMaxLimit);
...
udword NewtonPint::BatchCapsuleSweeps(PintSQThreadContext context, udword nb, PintRaycastHit* dest, const PintCapsuleSweepData* sweeps)
{
	...
	mBatchSweepData.m_castingShape = NewtonCreateCapsule(mWorld, sweeps->mCapsule.mRadius, sweeps->mCapsule.mRadius, height, 0, &aligment[0][0]);
	NewtonWaitForUpdateToFinish (mWorld);
	for (int i = 0; i < int (nb); i ++) {
		PintRaycastHit* const raycast = &dest[i];
...
/*void NewtonPint::ApplyActionAtPoint(PintObjectHandle handle, PintActionType action_type, const Point& action, const Point& pos)
{
	...
	else if(action_type==PINT_ACTION_IMPULSE)
	{
		...
		if (((cookie->m_extLinearImpulse.DotProduct3(cookie->m_extLinearImpulse)) == 0.0f) && ((cookie->m_extAngularImpulse.DotProduct3(cookie->m_extAngularImpulse)) == 0.0f)) {
