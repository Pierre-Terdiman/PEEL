...
bool AABBCollisionTree::Build(AABBTree* tree)
{
	...
	if(mNbNodes!=NbNodes)	// Same number of nodes => keep moving
	{
		mNbNodes = NbNodes;
		DELETEARRAY(mNodes);
		mNodes = ICE_NEW(AABBCollisionNode)[mNbNodes];
		CHECKALLOC(mNodes);
	}
...
bool AABBNoLeafTree::Build(AABBTree* tree)
{
	...
	if(mNbNodes!=NbTriangles-1)	// Same number of nodes => keep moving
	{
		...
		DELETEARRAY(mNodes);
		mNodes = ICE_NEW(AABBNoLeafNode)[mNbNodes];
		CHECKALLOC(mNodes);
	}
...
bool AABBQuantizedTree::Build(AABBTree* tree)
{
	...
	mNbNodes = NbNodes;
	DELETEARRAY(mNodes);
	AABBCollisionNode* Nodes = ICE_NEW(AABBCollisionNode)[mNbNodes];
	CHECKALLOC(Nodes);
	...
	// Quantize
	{
		mNodes = ICE_NEW(AABBQuantizedNode)[mNbNodes];
		CHECKALLOC(mNodes);
...
bool AABBQuantizedNoLeafTree::Build(AABBTree* tree)
{
	...
	mNbNodes = NbTriangles-1;
	DELETEARRAY(mNodes);
	AABBNoLeafNode* Nodes = ICE_NEW(AABBNoLeafNode)[mNbNodes];
	CHECKALLOC(Nodes);
	...
	// Quantize
	{
		mNodes = ICE_NEW(AABBQuantizedNoLeafNode)[mNbNodes];
		CHECKALLOC(mNodes);
