...
{
public:
HK_DECLARE_NONVIRTUAL_CLASS_ALLOCATOR(HK_MEMORY_CLASS_BASE,BackfacesCulledRayHitCollector);
...
void BackfacesCulledRayHitCollector::addRayHit( const hkpCdBody& cdBody, const hkpShapeRayCastCollectorOutput& hitInfo )
{
	...
	if (cdBody.getShape()->getType() == hkcdShapeType::TRIANGLE ) {
		...
		if (triangleNormal.dot3(invertTransformedHitNormal).getReal() < 0.0f) return;
	}
...
//	caps.mSupportArticulations			= true;
	...
	//mSupportBoxOverlaps
	//mSupportCapsuleOverlaps
	//mSupportConvexOverlaps
	//mSupportVehicles
}
...
// Custom allocator to double-check the returned amount of memory but it seems genuine with the default allocator
#define USE_CUSTOM_ALLOCATOR
#ifdef USE_CUSTOM_ALLOCATOR
class CustomMemory : public hkMemoryAllocator
{
	public:
	...
	virtual void blockFree( void* ptr, int size )
	{
		if(!ptr)
			return;
		char* bptr = (char*)ptr;
	...
	{
		u.m_allocated = mCurrentMemory;
	}

	virtual int getAllocatedSize(const void* obj, int nbytes) const
	{
		return 0;
	}
...
#ifdef USE_CUSTOM_ALLOCATOR
//	mMemoryRouter = hkMemoryInitUtil::initDefault( &gMyMemoryAllocator, hkMemorySystem::FrameInfo( 2 * 1024 * 1024 ) );
//	mMemoryRouter = hkMemoryInitUtil::initSimple( &gMyMemoryAllocator, hkMemorySystem::FrameInfo( 2 * 1024 * 1024 ) );
	mMemoryRouter = hkMemoryInitUtil::initHeapAllocator( &gMyMemoryAllocator, &gMyMemoryAllocator, null, hkMemorySystem::FrameInfo(mParams.mSolverBufferSize * 1024));
#else
...
/*	{
		...
		if(0 && desc.mGlobalBounds.IsValid())
		{
			Point m, M;
...
#ifdef USE_CUSTOM_ALLOCATOR
	return gMyMemoryAllocator.mCurrentMemory;
#endif
...
//	return Stats.m_inUse;
	udword n = Stats.m_entries.getSize();
	printf("\n");
	for(udword i=0;i<n;i++)
		printf("%s: %d\n", Stats.m_entries[i].m_allocatorName.cString(), Stats.m_entries[i].m_allocatorStats.m_inUse);
	}
...
udword Havok::BatchRaycasts(PintSQThreadContext context, udword nb, PintRaycastHit* dest, const PintRaycastData* raycasts)
{
	...
	while(nb--)
	{
		...
		output.reset();
//		hkVector4 culledRayDirection; culledRayDirection.setSub4(input.m_to, input.m_from);
//		BackfacesCulledRayHitCollector output(culledRayDirection);
...
//				HK_SET_OBJECT_COLOR( hkUlong(body->getCollidable()), 0xffff0000 );
		...
		//
		{
			...
			{
#ifdef MODIF
				info.m_position.set(1.0f * hkReal(b), 0.0f, 0.0f);
#else
...
#ifdef MODIF
						chainData->addConstraintInfoInBodySpace( hkVector4::getZero(), hkVector4( -1.0f, 0.0f, 0.0f) );
#else
